...
const EventDetailsScreen = ({ navigation, route }) => {
  ...
  const handleJoinEvent = async () => {
    console.log('🔍 EventDetailsScreen.handleJoinEvent - Début');
    ...
    console.log('   - eventData.organizer:', eventData?.organizer);
    console.log('   - isProcessingAction:', isProcessingAction);
    
    // Protection contre les clics multiples
    if (isProcessingAction) {
      console.log('⚠️ Action déjà en cours, clic ignoré');
      return;
    }
    
    // Obtenir l'ID de l'utilisateur de manière cohérente
    const userId = user?._id || user?.id;
    console.log('   - userId final:', userId);
    
    // Déterminer l'action à effectuer
    const buttonState = eventService.getJoinButtonState(eventData, userId);
    console.log('   - buttonState:', buttonState);
    console.log('   - action:', buttonState.action);
    
    if (!eventData) {
      console.log('❌ Pas de eventData');
    ...
    if (!eventId) {
      console.log('❌ Pas d\'eventId');
      Alert.alert('Erreur', 'ID de l\'événement non trouvé');
      return;
    }

    // Vérifier si l'utilisateur peut rejoindre
    const joinStatus = eventService.canUserJoinEvent(eventData, user?.id);
    console.log('   - joinStatus:', joinStatus);
    
    if (!joinStatus.canJoin) {
      console.log('❌ Ne peut pas rejoindre:', joinStatus.reason);
      Alert.alert('Impossible de rejoindre', joinStatus.reason);
      return;
    }

    // Déterminer le texte de confirmation selon l'action
    const isLeaveAction = buttonState.action === 'leave';
    const confirmTitle = isLeaveAction ? 'Quitter l\'événement' : 'Rejoindre l\'événement';
    const confirmMessage = isLeaveAction 
      ? `Voulez-vous vraiment quitter "${getEventTitle(eventData)}" ?`
      : `Voulez-vous vraiment rejoindre "${getEventTitle(eventData)}" ?`;
    const confirmButtonText = isLeaveAction ? 'Quitter' : 'Rejoindre';
    
    console.log('   - isLeaveAction:', isLeaveAction);
    ...
    Alert.alert(
      ...
      [
        ...
        { 
          ...
          onPress: async () => {
            try {
              console.log('🔄 DÉBUT DE L\'APPEL API via ButtonService');
              console.log('   - eventId:', eventId);
              console.log('   - action:', isLeaveAction ? 'LEAVE' : 'JOIN');
              console.log('   - Timestamp:', new Date().toISOString());
              
              // Marquer l'action comme en cours
              setIsProcessingAction(true);
              console.log('   - isProcessingAction mis à true');
              
              // Appeler la bonne fonction selon l'action
              const result = isLeaveAction 
                ? await buttonService.leaveEvent(
                    eventId,
                    (successResult) => {
                      console.log('✅ CALLBACK SUCCÈS LEAVE EXÉCUTÉ !');
                      console.log('   - successResult:', successResult);
                      console.log('   - Message:', successResult.message);
                      console.log('   - Timestamp:', new Date().toISOString());
                      
                      Alert.alert('Succès', successResult.message);
                      console.log('   - Alert affiché, rechargement des données...');
                      console.log('   - Appel de fetchEventDetails()...');
                      fetchEventDetails(); // Recharger les données
                      setIsProcessingAction(false); // Réinitialiser l'état
                      console.log('   - isProcessingAction remis à false (LEAVE)');
                    },
                    (errorResult) => {
                      console.log('❌ CALLBACK ERREUR LEAVE EXÉCUTÉ !');
                      console.log('   - errorResult:', errorResult);
                      console.log('   - isLoggedOut:', errorResult.isLoggedOut);
                      console.log('   - Timestamp:', new Date().toISOString());
                      
                      if (errorResult.isLoggedOut) {
                        console.log('   - Redirection vers Login...');
                        Alert.alert('Connexion requise', 'Vous devez être connecté pour quitter un événement');
                      } else {
                        console.log('   - Affichage de l\'alerte d\'erreur...');
                        Alert.alert('Erreur', errorResult.error);
                      }
                      setIsProcessingAction(false); // Réinitialiser l'état
                      console.log('   - isProcessingAction remis à false (LEAVE ERROR)');
                    }
                  )
                : await buttonService.joinEvent(
                    eventId,
                    (successResult) => {
                      console.log('✅ CALLBACK SUCCÈS JOIN EXÉCUTÉ !');
                      console.log('   - successResult:', successResult);
                      console.log('   - Message:', successResult.message);
                      console.log('   - Timestamp:', new Date().toISOString());
                      ...
                      fetchEventDetails(); // Recharger les données
                      setIsProcessingAction(false); // Réinitialiser l'état
                      console.log('   - isProcessingAction remis à false (JOIN)');
                    },
                    (errorResult) => {
                      console.log('❌ CALLBACK ERREUR JOIN EXÉCUTÉ !');
                      console.log('   - errorResult:', errorResult);
                      console.log('   - isLoggedOut:', errorResult.isLoggedOut);
                      console.log('   - Timestamp:', new Date().toISOString());
                      ...
                      setIsProcessingAction(false); // Réinitialiser l'état
                      console.log('   - isProcessingAction remis à false (JOIN ERROR)');
                    }
                  );
              console.log('✅ FIN DE L\'APPEL API joinEvent:');
              console.log('   - result:', result);
              console.log('   - Timestamp:', new Date().toISOString());
            } catch (error) {
              console.error('❌ ERREUR CRITIQUE handleJoinEvent:', error);
              console.log('   - Type d\'erreur:', error.constructor.name);
              console.log('   - Message:', error.message);
              console.log('   - Stack:', error.stack);
              Alert.alert('Erreur', 'Une erreur est survenue: ' + error.message);
              setIsProcessingAction(false); // Réinitialiser l'état en cas d'erreur
              console.log('   - isProcessingAction remis à false (CATCH ERROR)');
            }
          }
        }
      ]
